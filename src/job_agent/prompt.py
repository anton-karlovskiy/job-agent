...
def build_task_prompt(applicant: ApplicantData, job_url: str, dry_run: bool) -> str:
    ...
    Args:
        applicant: Loaded applicant data containing YAML data and resume text.
...
wait for the suggestion list to appear, then click the correct option from the list. \
Never leave a combobox after typing without confirming a selection from the dropdown. \
If the suggestion list appears but contains no option that matches the profile data \
AND the field is required, call `ask_human` to surface it.
9. Call `ask_human` ONLY for hard blockers you cannot bypass: a CAPTCHA, an unexpected login wall, \
an MFA prompt, a required field with absolutely no inferable answer (e.g. an internal employee ID \
or a referral code), or a required dropdown whose available options don't match the applicant's \
profile (e.g. a US-state selector when the applicant has no US address). \
Describe exactly what you encountered and what the user needs to do.
10. After completing the form (or reaching the dry-run stopping point), use the `done` action with \
